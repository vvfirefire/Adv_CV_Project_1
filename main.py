...
def main():
    ...
    val_dataset = CelebA(args.data, 'validation.txt', transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ]))
    test_dataset = CelebA(args.data, 'validation.txt', transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ]))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    # print (" the accuracy for all the attribute is ", top1)
    bar.finish()
...
def validate(val_loader, model, criterion):
    ...
    losses = [AverageMeter() for _ in range(40)]
    top1 = [AverageMeter() for _ in range(40)]
    # attribute_acc =[0.0 for_ in range (40)]
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            for j in range(len(output)):
                loss.append(criterion(output[j], target[:, j]))
                prec1.append(accuracy(output[j], target[:, j], topk=(1,)))
                # attribute_acc[j] += prec1[j][0].item()
                losses[j].update(loss[j].item(), input.size(0))
                top1[j].update(prec1[j][0].item(), input.size(0))
        ...
        bar.next()
        # for att in attribute_acc: att = att/num
    bar.finish()
